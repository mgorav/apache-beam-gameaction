 ...
 */
public class InjectorUnboundedSourceCreator extends UnboundedSource<GameActionInfo, InjectorUnboundedSourceCreator.Checkpoint> {

  /**
   * Checkpointing an {@link InjectorUnboundedSourceCreator} requires remembering our
  ...
  public InjectorUnboundedSourceCreator() {
    this(new InjectorIterator.SourceConfig(null /* no limit */, 15, 5000, 8000));
  }

  private InjectorUnboundedSourceCreator(InjectorIterator.SourceConfig config) {
    this.config = config;
  }

  @Override
  public List<InjectorUnboundedSourceCreator> split(int desiredNumSplits, PipelineOptions options)
      throws Exception {
    int numSplits = Math.min(desiredNumSplits, config.numTeams);
    ArrayList<InjectorUnboundedSourceCreator> splits = new ArrayList<>(numSplits);
    for (InjectorIterator.SourceConfig config : config.split(numSplits)) {
      splits.add(new InjectorUnboundedSourceCreator(config));
    }
    return splits;
  }
  ...
  private static class InjectorUnboundedReader extends UnboundedReader<GameActionInfo> {
    ...
    private final InjectorUnboundedSourceCreator source;
    private final Random random;
    ...
    public InjectorUnboundedReader(InjectorUnboundedSourceCreator source, @Nullable Checkpoint initialCheckpoint) {
      this.source = source;
